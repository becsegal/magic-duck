...
class Bundler extends broccoli_plugin_1.default {
    ...
    get bundlerHook() {
        if (!this.cachedBundlerHook) {
            const internalWebpackConfig = {};
            if ([...this.options.packages.values()].find(pkg => pkg.forbidsEval)) {
                internalWebpackConfig.devtool = 'source-map';
            }
            let extraWebpackConfig = lodash_1.mergeWith(internalWebpackConfig, ...[...this.options.packages.values()].map(pkg => pkg.webpackConfig), (objValue, srcValue) => {
                // arrays concat
